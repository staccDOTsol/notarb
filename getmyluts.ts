...
process.env.SEARCHER ? connection = connection2 : connection = connection
  ...
  const payer = (
    Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync((process.env.NODE_ENV == 'production' ? '/home/ubuntudunn' : '/home/ubuntudunn') + '/notjaregm.json').toString()))));
