...
// invalid cache. I will recommend using a paid RPC endpoint.
    ...
    var connection2= new Connection("https://solana-mainnet.g.alchemy.com/v2/Zf8WbWIes5Ivksj_dLGL_txHMoRA7-Kr", {skipPreflight: false});
...
let min = -0.0001//.9 * ( reserve.stats.borrowFeePercentage * 100)
    ...
    for (var SOL_MINT of mints){
      ...
      SOL_MINT = mints[Math.floor( rando(0, 1, "float") * mints.length)]
              if (SOL_MINT != USDC_MINT && !baddies.includes(SOL_MINT+USDC_MINT) &&  !baddies.includes(USDC_MINT+SOL_MINT)){
      let dothethings = []
      cba++
      try {
        let tokenAccount
