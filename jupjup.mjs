...
// This is a free Solana RPC endpoint. It may have ratelimit and sometimes
// invalid cache. I will recommend using a paid RPC endpoint.
let  connection = new Connection((process.env.NODE_ENV == 'production' ? 'http://localhost:8899' : 'https://solana-mainnet.g.alchemy.com/v2/ETWO1_-exD_tuIyq9YTW9d37nAvNT7XQ'), {skipPreflight: false});
   ...
   //initial = initial + 0.1 * 10 ** 6
        ...
        try {
            ...
            if (!baddies.includes(USDC_MINT+SOL_MINT)){
             ...
             let solToUsdc
                try {
         usdcToSol = await getCoinQuote(USDC_MINT, SOL_MINT, Math.floor(initial * 1.002));
