...
let min = ( reserve.stats.borrowFeePercentage * 100)
      ...
      try {
        ...
        try {
            ...
            if (!baddies.includes(USDC_MINT+SOL_MINT)){
                await sleep(Math.random()*(Math.random()*5000)+250)
   ...
   if ( new Date().getTime() % 100 <= 25 )   console.log((initial / 10 ** dec * reserve.stats.assetPriceUSD).toString() + ' initial, ' + returns.toString() + '% yield on badboi ' + USDC_MINT + ' <-> ' + SOL_MINT+', time since last arb checked: ' + diff.toString() + 's, avg last 60 diffs: ' + nowavg.toString() + ', lendiffs: ' + avgs.length.toString()+ ', lenbaddies: ' + baddies.length.toString())
