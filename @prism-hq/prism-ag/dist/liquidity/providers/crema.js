...
function getCremaPools(connection) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < programAccounts.length; i++)
            promises.push(web3_js_1.PublicKey.findProgramAddress([programAccounts[i].publicKey.toBuffer()], new web3_js_1.PublicKey(types_1.CREMA_PROGRAM_ID)));
        let pools = {};
        yield Promise.all(promises).then(results => {
            for (let i = 0; i < programAccounts.length; i++) {
                ...
                let pool = Object.assign(Object.assign({}, decoded), { authority: results[i][0], provider: "crema" });
